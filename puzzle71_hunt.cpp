/*
 * puzzle71_hunt.cpp - Ultra High-Performance Bitcoin Puzzle 71 Key Hunter
 * 
 * Target: RIPEMD160 hash f6f5431d25bbf7b12e8add9af5e3475c44a0a5b8
 * Range: 2^70 to 2^71-1
 * 
 * OPTIMIZATIONS (vs original):
 *  1. Device memory (cudaMalloc) instead of managed memory — no page faults
 *  2. Larger GPU_BATCH_SIZE (1024) — amortizes fp_inv cost over more keys
 *  3. Shared memory G-table for first 64 entries — fast SM-local access
 *  4. Early RIPEMD160 rejection — checks first 4 bytes before full hash
 *  5. Dedicated fp_sqr via u256_sqr_full — ~25% fewer field multiplies
 *  6. Double-buffered kernel launches with CUDA streams — GPU never idles
 *  7. Proper T4 occupancy: 40 SMs × 2048 threads/SM tuning
 *  8. Endomorphism: 6 hash checks per EC operation
 *  9. Precomputed target_h0 for early rejection filter
 * 10. Reduced per-thread stack by streaming batch computation
 */
...
__device__ uint256_t d_SECP256K1_BETA2;
...
__attribute__((unused)) static uint256_t s_SECP256K1_BETA2;
...
static uint256_t s_SECP256K1_BETA2;
static point_t G_TABLE[BATCH_SIZE + 1];

#define G_POINT s_G_POINT
#define SECP256K1_BETA2 s_SECP256K1_BETA2
...
static uint256_t h_SECP256K1_BETA2;
...
static void save_result(const uint256_t &privkey, int endo_type) {
...
static void precompute() {
    ...
    fp_sqr(h_SECP256K1_BETA2, SECP256K1_BETA);
...
__global__ void search_kernel(uint256_t start_key_base, uint64_t keys_per_thread, 
                              int *found_flag, uint256_t *found_key, int *found_type) {
...
int main(int argc, char **argv) {
    ...
    cudaMalloc(&d_found_type, sizeof(int));
    int h_found_flag = 0;
    ...
    while (g_running.load() && !h_found_flag) {
        ...
        search_kernel<<<blocks, threads_per_block, 0, streams[stream_idx]>>>(
            start, keys_per_thread, d_found_flag, d_found_key, d_found_type);
        ...
        if (h_found_flag) {
            ...
            cudaMemcpy(&h_found_type, d_found_type, sizeof(int), cudaMemcpyDeviceToHost);
            g_found.store(true);
            save_result(h_found_key, h_found_type);
            break;
        }
    }
